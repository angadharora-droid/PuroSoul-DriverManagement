      ...
      <div className="mx-auto max-w-md animate-fade-in-up">
        ...
        <div className="overflow-hidden rounded-2xl border border-brand-200 bg-white shadow-card">
          ...
          <div className="px-6 pb-6">
            ...
            {handover.parties?.length > 0 && (
              <div className="mt-3 overflow-hidden rounded-xl border border-slate-200">
                <p className="border-b border-slate-200 bg-slate-50/80 px-4 py-2 text-[11px] font-semibold uppercase tracking-wide text-slate-500">
                  Party-wise amounts
                </p>
                <div className="max-h-56 overflow-y-auto">
                  {handover.parties.map((p) => (
                    <div key={p.name} className="flex items-center justify-between gap-3 border-b border-slate-100 px-4 py-2.5 text-sm last:border-0">
                      <span className="min-w-0 truncate font-semibold text-slate-800">
                        {p.name}
                        {p.count > 1 && <span className="ml-1.5 font-normal text-slate-400">× {p.count}</span>}
                      </span>
                      <span className="tnum shrink-0 font-bold text-slate-900">{formatINR(p.amount)}</span>
                    </div>
                  ))}
                </div>
              </div>
